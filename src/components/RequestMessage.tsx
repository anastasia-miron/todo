...
const RequestMessage: React.FC<Props> = (props) => {
    ...
    useLayoutEffect(() => {
        (async () => {
            setIsLoading(true);
            const response = await apiService.get<MessageModel[]>(`/requests/${id}/messages`, { signal });
    ...
    const handleSendMessages = async (ev: FormEvent) => {
        ev.stopPropagation();
        ev.preventDefault();
        const response = await apiService.post<MessageModel>(`/requests/${id}/messages`, { content }, { signal });
        if (signal.aborted) return;
        if (response.success) {
    ...
    const canSendMessage = user!.id === request.beneficiary_id || user!.id === request.volunteer_id;
        ...
        <article>
            ...
            {canSendMessage && (
                <div className="message-input">
                    <form onSubmit={handleSendMessages} className="message-input__container" role="group">
                        <input
                            type="text"
                            required
                            value={content}
                        ...
                        />
                        <button type="submit" className="outline contrast message-input__button" disabled={content.length === 0}>
                            <SendHorizontalIcon />
                        </button>
                    </form>
                </div>
